...
class Home extends Component {
  ...
  // GET ALL ITEMS
  componentDidMount(){
    axios.get('http://ishowcase.great-site.net/php/sellread.php')
    .then(response => {
  ...
  productList(){
    if(Object.keys(this.state.products).length === 0){
      ...
      return this.state.products.map((object, i) => {
        if(object.img){
          return <div key={i} className="homeview"><img src={"http://ishowcase.great-site.net/php/uploads/"+object.img } alt="img"></img><h3><Link to={ "/viewsell/"+object.id }>{object.title}</Link></h3><p>${object.price}</p></div>
        } else {
          return <div key={i} className="homeview"><img src="http://ishowcase.great-site.net/php/default.jpg" alt="img"></img><h3><Link to={ "/viewsell/"+object.id }>{object.title}</Link></h3><p>${object.price}</p></div>
        }
      })
    }
  }
